...
API_ID = int(environ.get('API_ID', '12775397'))
API_HASH = environ.get('API_HASH', '97217dbd74d74706a12bb1cbb0c40201')
BOT_TOKEN = environ.get('BOT_TOKEN', "7112495962:AAE55mo-qJ5fh43bnvDbz5P5_jkKmpQzAUE")
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6419308127').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001959951947').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '6419308127').split()]
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002082073586') # support group id ( make sure bot is admin )
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002082073586') # request channel id ( make sure bot is admin )
...
# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://althafmkallu:123meallu@cluster0.ouajdtj.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002061898299')) #Log channel id ( make sure bot is admin )
...
FQDN = str(getenv('FQDN', BIND_ADRESS)) if not ON_HEROKU or getenv('FQDN') else 'mybbbot-8b8cabe135e6.herokuapp.com/'
...
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002061898299'))
